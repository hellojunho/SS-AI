...
def summarize_chat(content: str, date: datetime) -> str:
    ...
    system_prompt = "다음 대화 기록을 하루 단위로 요약하세요. 핵심 개념과 학습 포인트만 간결하게 정리하세요."
    messages = [
        {"role": "system", "content": system_prompt},
        {
            "role": "user",
...
def _normalize_quiz_payload(payload: dict) -> dict:
    ...
    if correct_index in range(len(normalized_choices)):
        tagged = [
            {"choice": choice, "is_correct": idx == correct_index}
            for idx, choice in enumerate(normalized_choices)
        ]
        random.shuffle(tagged)
        normalized_choices = [item["choice"] for item in tagged]
        correct_index = next((idx for idx, item in enumerate(tagged) if item["is_correct"]), -1)

    correct = normalized_choices[correct_index] if correct_index in range(4) else ""
    wrong = [choice for idx, choice in enumerate(normalized_choices) if idx != correct_index]
